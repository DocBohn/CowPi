...
\subsection{Hello World for the LCD1602 Display Module}
    ...
    We also specify whether to use SPI or I$^2$C.\footnote{As of v0.3, if using SPI, the CowPi library will only work with the AdaFruit LCD1602 ``backpacks.'' If using I$^2$C, the CowPi library will work with both the AdaFruit ``backpacks'' and the ubiquitous and cheap serial adapters. More general SPI-LCD1602 support will be part of v0.4.}
    ...
    if the wiring is good, then run a program to scan the I$^2$C for peripherals and report their addresses.\footnote{Such a program is available on the Arduino website: \url{https://playground.arduino.cc/Main/I2cScanner/}.}
